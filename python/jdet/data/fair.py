...
class FAIRDataset(DOTADataset):
    ...
    def __init__(self,**kwargs):
        ...
        for img_info in self.img_infos:
            boxes = img_info["ann"]["bboxes"] # bboxes = [x_center,y_center,w,h,angle].astype(np.float32) n*5
            if boxes.shape[0]==0:
                continue
            print(f'定位在fair.py第18行代码输出:boxes={boxes}')
            w,h = boxes[:,2],boxes[:,3] #w,h 是一组标量
            print(f'定位在fair.py第20行代码输出:w,h={w},{h}')
            area = w*h
            print(f'定位在fair.py第21行代码输出:area={area}')
            index = area>1. #area丢弃阈值
            print(f'定位在fair.py第23行代码输出:index={index}')
            img_info["ann"]["bboxes"] = img_info["ann"]["bboxes"][index] #丢弃面积<1像素的bbox和label
    ...
    def _balance_categories(self):
        ...
        for k,d in cate_dict.items():
            ...
            new_d = d*l1+d[:l2] #重复性采样数据，保持数据平衡
