...
class BasicBlock(nn.Module):
    ...
    def execute(self, x):
        ...
        out = self.relu(out)
        return out
# bottleneck 通过降维度减少模型参数，增加模型深度，提升模型表征学习能力，减少训练时间。但是一般是通过升维提高模型表征学习能力，这里的降维操作属实有点不太理解。
#将basicblock中的第一个conv3x3和最后一个conv3x3换成conv1x1，减少参数量
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, return_stages=["layer4"],frozen_stages=-1,norm_eval=True,num_classes=None, groups=1, width_per_group=64, replace_stride_with_dilation=None, norm_layer=None):
        ...
        self._norm_layer = norm_layer # batchnorm2d
        self.inplanes = 64
        self.dilation = 1
        ...
        self.layer1 = self._make_layer(block, 64, layers[0]) #第一层不会下采样
        self.layer2 = self._make_layer(block, 128, layers[1], stride=2, dilate=replace_stride_with_dilation[0]) # 下采样
        self.layer3 = self._make_layer(block, 256, layers[2], stride=2, dilate=replace_stride_with_dilation[1])#下采样
        self.layer4 = self._make_layer(block, 512, layers[3], stride=2, dilate=replace_stride_with_dilation[2])#下采样
...
# _make_layer 可能结构:(downsampling layer),block(inplanes=64),block*i(64*expansion)
    def _make_layer(self, block, planes, blocks, stride=1, dilate=False):
        norm_layer = self._norm_layer # default=batchnorm2d,you can set other normlayer
        downsample = None
        previous_dilation = self.dilation # self.dilation=1
        if dilate:
            self.dilation *= stride
            stride = 1
        if ((stride != 1) or (self.inplanes != (planes * block.expansion))): # in_channel != (out_channel)* block.expansion:basic_block.expansion=1/bottleneck.expansion=4 
            downsample = nn.Sequential(conv1x1(self.inplanes, (planes * block.expansion), stride), norm_layer((planes * block.expansion)))
        layers = []
        layers.append(block(self.inplanes, planes, stride, downsample, self.groups, self.base_width, previous_dilation, norm_layer)) #self.inplanes=64
        self.inplanes = (planes * block.expansion) #针对basicblock后续加上bottleneck后，它的通道是basicblock的planes*4得到
        for _ in range(1, blocks): #blocks指需要设置的层数
    ...
    def execute(self, x):
        ...
        x = self.relu(x)
        x = self.maxpool(x)
        for i in range(1,5): #layer1-4(x)
            name = f"layer{i}"
            x = getattr(self,name)(x)
            if name in self.return_stages:# 如果是倒数第二层，append其output
                outputs.append(x) 
