...
class Config(OrderedDict):
    ...
    def _load_dict_from_file_no_base(filename):
        if check_file(filename,ext=[".yaml"]):
            with open(filename, "r") as f:
                cfg = yaml.safe_load(f.read()) #yaml格式的配置文件可以直接通过yaml.safe_load直接加载

        elif check_file(filename,ext=[".py"]):
            f_dir = os.path.dirname(filename) # os.path.dirname 作用是去掉路径中的最后一级目录 
            f_name = os.path.basename(filename) # os.path.basename 返回
            module_name = f_name[:-3] # fair1m_1_5_preprocessed_config

            # temp_module_name = osp.splitext(temp_config_name)[0]
            sys.path.insert(0, f_dir) # 将所要导入的文件的父级目录插入到当前的系统目录列表中
            # Config._validate_py_syntax(filename)
            mod = import_module(module_name) #导入该配置文件 类似：from configs/preprocessed import fair1m_1_5_preprocessed_config
            sys.path.pop(0) # 在将当前系统目录列表中的第一个元素删除，即导入文件的父级目录
            ...
            del sys.modules[module_name] #删除当前导入的配置模块
        else:
            assert(False)
        return cfg
    ...
    """深度优先遍历字典树结构"""
    def dfs(self, cfg_other): #深度优先遍历config文件
        if isinstance(cfg_other,dict):
            now_cfg = Config() #从加载文件中初始化now_cfg
            ...
            now_cfg = copy.deepcopy(cfg_other) # 如果不是dict/list格式，即如果是key:value格式时，直接拷贝到now_cfg中
        return now_cfg
