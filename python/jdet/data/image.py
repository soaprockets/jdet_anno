...
class ImageDataset(Dataset):
    ...
    def __init__(self,images_file=None,
        ...
        if isinstance(transforms,list):#判断transforms是否为list
    ...
    def _load_images(self,images_file,images_dir):
        if (not images_file): #image_file is not null 
            images = []
            ...
            pass 
        elif isinstance(images_file,str):# 也有可能要加载pkl文件，图像信息存在字典中
    ...
    def collate_batch(self,batch):
        ...
        for image,ann in batch:
            height,width = image.shape[-2],image.shape[-1] # image.shape=(c,h,w)
            max_width = max(max_width,width)
            max_height = max(max_height,height)
            imgs.append(image)
            anns.append(ann)
        N = len(imgs)
        batch_imgs = np.zeros((N,3,max_height,max_width),dtype=np.float32) # 生成 batch批处理的图像矩阵(n,c,max_h,max_w)同维度的零矩阵，
        for i,image in enumerate(imgs):
            batch_imgs[i,:,:image.shape[-2],:image.shape[-1]] = image # batch_img 矩阵数据填充
        """
        image维度统一从(c,h,w)扩展到(b,c,h,w)
        """
