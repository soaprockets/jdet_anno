...
def to_jt_var(data):
    ...
    def _to_jt_var(data):
        if isinstance(data,(list,tuple)):  
            data =  [_to_jt_var(d) for d in data] # 递归使用_to_jt_var()方法，将data传递到底层 data = jt.array(data)
        elif isinstance(data,dict):
            data = {k:_to_jt_var(d) for k,d in data.items()}
        elif isinstance(data,numpy.ndarray):
            data = jt.array(data)
        elif not isinstance(data,(int,float,str,np.ndarray)): #如果数据不是int,float,str,np.array类型，抛出异常
...
def is_img(f):
    ext = os.path.splitext(f)[1] #将文件名与扩展名分开，取扩展名
