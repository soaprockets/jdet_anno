...
def poly_to_rotated_box_single(poly):
    ...
    pt1 = (poly[0], poly[1]) # 向量形式
    pt2 = (poly[2], poly[3])
    pt3 = (poly[4], poly[5])
    pt4 = (poly[6], poly[7])

    edge1 = np.sqrt((pt1[0] - pt2[0]) * (pt1[0] - pt2[0]) +
                    (pt1[1] - pt2[1]) * (pt1[1] - pt2[1]))   #计算第一条边长度
    edge2 = np.sqrt((pt2[0] - pt3[0]) * (pt2[0] - pt3[0]) +
                    (pt2[1] - pt3[1]) * (pt2[1] - pt3[1]))  #计算第二条边长度

    width = max(edge1, edge2) # 图像为横幅图像，width>height
    height = min(edge1, edge2)

    angle = 0
    if edge1 > edge2:
        angle = np.arctan2(  # 1/tan(x) tan(y/x)计算角度，则arctan(y) =angle
...
def poly_to_rotated_box_np(polys):
    ...
    批处理多个bboxes并转成np.float32类型，基于poly_to_rotated_box_single(poly)函数
    """
    rotated_boxes = []
    for poly in polys:
